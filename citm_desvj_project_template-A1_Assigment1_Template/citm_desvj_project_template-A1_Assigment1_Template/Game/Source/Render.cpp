...
bool Render::Awake(pugi::xml_node& config)
{
	...
	font = TTF_OpenFont("Assets/Fonts/arial/arial.ttf", 200);
	return ret;
}
...
// Blit to screen
bool Render::DrawTexture(SDL_Texture* texture, int x, int y, bool isFlipped, const SDL_Rect* section, float speed, double angle, int pivotX, int pivotY, int zoomFactor, bool useCamera) const
{
	bool ret = true;
...
bool Render::DrawText(const char* text, int posx, int posy, int w, int h, int r, int b, int g) {
...
bool Render::LoadState(pugi::xml_node node) {
	...
	camera.x = node.child("camera").attribute("y").as_int();
	return true;
}
